import random
import math
from mesa.discrete_space import ContinuousAgent
from helper import cell_dist


class Animal(ContinuousAgent):
    """Base animal class."""
...
class Rabbit(Animal):
    ...
    def move(self):
        """Move."""
        print(f"Rabbit coords: {self.cell.coordinate[0]}, {self.cell.coordinate[1]}")

        possible_cells = self.cell.get_neighborhood(include_center=True)
...
class Fox(Animal):
    ...
    def feed(self):
        ...
        if rabbits:
            ...
            self.energy += 3 #TODO make this a hyperparameter
            to_be_eaten.remove()


    def move(self):
        """Move behaviour."""
        # collect all cells 3 away (inclusive) where there is a rabbit
        prey_cells = self.cell.get_neighborhood(radius=20, include_center=True).select(
            lambda cell: any(isinstance(obj, Rabbit) for obj in cell.agents))

        print(f"Fox coords: {self.cell.coordinate[0]}, {self.cell.coordinate[1]}")
        ...
        if not prey_cells:
            ...
            # self.cell = prey_cells.select_random_cell()
            ...
            move_x = closest_prey_coords[0] - self.cell.coordinate[0]
            move_y = closest_prey_coords[1] - self.cell.coordinate[1]
            print(f"move_x: {move_x},  move_y: {move_y}")
            self.cell.coordinate = [self.cell.coordinate[0] + move_x, (self.cell.coordinate[1] + move_y) % self.model.grid.width]


        # target_cells = (
        #     prey_cells if len(prey_cells) > 0 else self.cell.neighborhood
        # )
        ...
        # # collect all cells 3 away (inclusive) where there is a rabbit
        # prey_cells = self.cell.get_neighborhood(radius=10, include_center=True).select(
        #     lambda cell: any(isinstance(obj, Rabbit) for obj in cell.agents))
        ...
        #         self.feed()
        #     elif move_x == 0:
        #         self.cell.coordinate = [self.cell.coordinate[0], (self.cell.coordinate[1] + int(math.copysign(1, move_y))) % self.model.grid.width]
        #     elif move_y == 0:
        #         self.cell.coordinate = [(self.cell.coordinate[0] + int(math.copysign(1, move_x))) % self.model.grid.height, self.cell.coordinate[1]]
